...
function Dashboard() {
  ...
  const statCards = [
    {
      title: role === "admin" ? "Total Semua Tiket" : "Total Tiket Saya",
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className="text-base font-semibold text-emerald-900">{fullName}</p>
        </div>
      </div>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] gap-8 items-center">
          <div>
            ...
            <p className="text-sm text-emerald-100/80 max-w-xl mb-5">
              Pantau status tiket pengaduan Anda dan koordinasikan penyelesaian
            ...
            <button
              type="button"
              onClick={() =>
                navigate(role === "admin" ? "/tickets-admin" : "/tickets")
              }
            ...
            >
              {role === "admin" ? "Kelola Semua Tiket" : "Lihat Tiket Saya"}
            </button>
          </div>
            ...
            {statCards.map((card) => (
              ...
              >
                ...
                <div className="mt-4 h-1.5 w-full rounded-full bg-emerald-50 overflow-hidden">
                  <div
                    className={`h-full w-2/3 rounded-full bg-gradient-to-r ${card.accent}`}
                  />
                </div>
              </div>
            ))}
